...
def build_model_class(class_path, class_name, class_info, overwrite=False):
    ...
    if is_emb:
        ...
        buf.write("        ''' Create a new " + cls_name + " instance '''\n")
    ...
    for field_name in field_names:
        ...
        buf.write(_make_getter_code(cls_name, field_name))
        buf.write(_make_setter_code(cls_name, field_name, field_info))
    ...
    if relations and len(relations) > 0:
        ...
        for rel_name in relations.keys():
            ...
            buf.write(_make_relational_getter_code(cls_name, rel_name, rel_info))
            buf.write(_make_relational_setter_code(cls_name, rel_name, rel_info))
    ...
    if not is_emb:
        ...
        buf.write("        ''' Ensure that all required fields are set and persist this object'''\n")
        for field_name in field_names:
            field_info = fields[field_name]
    ...
    if old_htag != new_htag or overwrite:
        ...
        try:
            ...
            print "ERROR: Could not write model class: %s" % e
...
def _make_relational_getter_code(cls_name, relation_name, relation_info):
    ...
    code += "        ''' Base relational getter for " + cls_name + "." + relation_name + " '''\n"
...
def _make_relational_setter_code(cls_name, relation_name, relation_info):
    ...
    code += "        ''' Base relational setter for " + cls_name + "." + relation_name + " '''\n"
    if multi:
...
if __name__ == "__main__":
    ...
    from pyutils.utils.config import ConfigParser, Config
    import time
    import sys
    ...
    def print_help():
        ...
        print "Usage:"
        print "--------------------------------------------------------------------------"
        print "python build_model.py -m MODELFILE -p CLASSPATH [-c CONFIGFILES] [--force]"
        print "Where: "
        print "  MODELFILE    The model config file to parse"
        print "  CLASSPATH    The directory where generated class files will end up"
        print "  CONFIGFILES   General config files to use, commat-separated"
        print "  --force      Forcefully regenerate all files"
        print "--------------------------------------------------------------------------"
    ...
    if "-m" not in sys.argv or "-p" not in sys.argv:
        print "ERROR: Missing parameters\n"
        print_help()
        exit()
    model_file = sys.argv[sys.argv.index('-m') + 1]
    class_path = sys.argv[sys.argv.index('-p') + 1]
    config_files = sys.argv[sys.argv.index('-c') + 1] if "-c" in sys.argv else None
    ...
    if config_files:
        for config_file in config_files.split(","):
            conf.update(ConfigParser().parse(config_file.strip()).get())
    model = ConfigParser().parse(model_file).get()
    conf['model'] = model
    Config.init_with_dict(conf)
    ...
    if db_config:
        print " > Updating db indexes"
        Database.get_instance(host=db_config['host'], port=db_config['port'], db_name=db_config['name'])
        Database.build_indexes(model, no_print=True)
